class Score < ApplicationRecord
  ...
  def self.get_searched_score_in_current_page(selected_date, start_date, current_user)
    if start_date.between?(Date.new(start_date.year, 12, 18), Date.new(start_date.year, 12, 31)) && selected_date.include?("12-")
      self.find_by(user_id: current_user.id,  start_time: "#{start_date.year}-#{selected_date}")
    elsif start_date.between?(Date.new(start_date.year, 12, 18), Date.new(start_date.year, 12, 31)) && selected_date.include?("1-")
      self.find_by(user_id: current_user.id,  start_time: "#{start_date.year + 1}-#{selected_date}")
    elsif start_date.between?(Date.new(start_date.year, 1, 1), Date.new(start_date.year, 1, 16)) && selected_date.include?("12-")
      self.find_by(user_id: current_user.id,  start_time: "#{start_date.year - 1}-#{selected_date}")
    elsif start_date.between?(Date.new(start_date.year, 1, 1), Date.new(start_date.year, 1, 16)) && selected_date.include?("1-")
      self.find_by(user_id: current_user.id,  start_time: "#{start_date.year}-#{selected_date}")
    else
      self.find_by(user_id: current_user.id,  start_time: "#{start_date.year}-#{selected_date}")
    end
